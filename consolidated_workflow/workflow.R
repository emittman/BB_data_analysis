...
analyses = analyses(
 ...
 #   samples_logodds_reduced_3_8 = run_mcmc_logodds(..dataset.., chains = 16, iter = 3000, warmup = 1500),
     s_tpvary = run_mcmc_generic(data_5_4, chains = 4, p = c(.8,.2), iter = 1000, warmup = 800, "glfp_tp2_vary.stan"),
     s_tpsigvary = run_mcmc_generic(data_5_4, chains = 4, p = c(.8,.2), iter = 1000, warmup = 800, "glfp_sig2_tp2_vary.stan")
  ),
  datasets = datasets
)
