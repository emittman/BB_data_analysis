...
\begin{equation}
f(t|\pi, t_{p1},\sigma_1, t_{p2}, \sigma_2) = \pi f_1(t|t_{p1},\sigma_1)\left(1-F_2(t|t_{p2},\sigma_2)\right) + f_2(t|t_{p2},\sigma_2)\left(1-\pi F_2(t|t_{p2},\sigma_2\right).
\end{equation}
...
\op{median}\left\{\op{GLFP}\left(\tilde{t}_m|\pi_{g}^{(s)},t_{p_{1}g}^{(s)},\sigma_{1g}^{(s)},t_{p_{2}g}^{(s)},\sigma_{2g}^{(s)}\right);s=1,\ldots,S \right\},
\end{equation}
...
period of 5 months. Manufactures of hard drives (and most other
...
\label{global-avg}
In Section \ref{model-assessment}, to illustrate the concept of shrinkage in our hierarchical model, we refer to a `global average' which represents an average GLFP cdf for the entire population, $\op{GLFP}\left(\bar{\pi},\mu_1,\sigma_1,\bar{\mu}_2,\bar{\sigma}_2\right)$. For the parameters that vary across drive models, we select values corresponding to the medians of the hierarchical distributions (Eq. \ref{eq:hier-model}) conditional on the hyperparameters. In particular, setting
$$\bar{\pi}=\op{logit}^{-1}(\eta_{\pi}),\;\bar{\mu}_2=\eta_{t_2} - m_{\sigma_2}\Phi^{-1}(.2) \mbox{ and } \bar{\sigma}_2= m_{\sigma_2}.$$
Let $F(\cdot|a,b),\;F^{-1}(\cdot|a,b)$ denote the cdf and inverse cdf, respectively, for a log-normal distribution with log-location parameter $a$ and log-scale parameter $b$. Then
$$m_{\sigma_2}=F^{-1}[0.5 \cdot F(1|\eta_{\sigma_2},\tau_{\sigma_2})|\eta_{\sigma_2}, \tau_{\sigma_2}],$$
...
We estimate the global average pointwise, using draws from the joint posterior distribution, $\left(\eta_\pi^{(s)}, \mu_1^{(s)}, \sigma_1^{(s)}, \eta_{t_2}^{(s)},\eta_{\sigma_2}^{(s)},\tau_{\sigma_2}^{(s)}\right)$, $s=1,2,\ldots,S$. The computation is thus similar to that shown in Equation \ref{pointwise-medians}.
