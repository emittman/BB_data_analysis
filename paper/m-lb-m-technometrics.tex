...
\end{equation*}
\begin{equation*}
\sigma_{2g} \ind \op{Log-normal} \left( \eta_{\sigma_2}, \tau^2_{\sigma_2}\right)\op{Tr}\left(0, 1\right)
\end{equation*}
\begin{equation}
\label{eq:hier-model}
...
The set of model specifications is chosen based data, interpretation of the model, as well as estimation considerations.  sub-population-model-specific parameters for the the wearout failure mode $(t_{p_{2}g},\sigma_{2g})$, and the proportion defective $(\pi_g)$, are considered as a means to account for heterogeneity across sub-populations in the right tails of the failure distribution.  Going from a common model for all sub-populations and gradually increasing the complexity of the model, Model 4 is the most flexible.  Model 4 allows for the probability of infant mortality as well as the shape and scale parameters for the wearout failure mode to vary by sub-population.
...
% We can compare the 19 simulated data sets $y_{rep,g, i}$ to the observed data $y_{g,i}$ by using them to re-estimate the K-M estimator. 
...
As discussed in Section \ref{sec:Weibull parameterization}, we use a parameterization of the Weibull distribution in terms of a chosen quantile, making prior elicitation straightforward. Posterior sampling immediately gives posterior distributions for these quantiles, which in reliability applications is typically more informative than a measure of central tendency. If the selected model includes varying $t_{p2g}$ across sub-poputations, side-by-side plots of credible intervals provides a way to compare lifetime of non-defective units. Depending on a user's application, a similar plot for the $\pi_g$'s as a comparison tool may be a relevant parameter of interest.
