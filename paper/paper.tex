...
for this, we can posit a model for this depreciation and instead of
TTF, we can consider the value at replacement. The IRS considered
computer hardware to be ``5-year'' equipment.(form 4562) Using``the declining
balance method'' the rate of depreciation is 40\% per year.

Let $L(t) = e^{-.4t}$ represent the value at the time of failure
relative to its original value. We can then define the predicted loss
of a unit as its value at time of failure.
...
\begin{figure}[H]
  ...
  \caption{Left: Posterior predictive TTF (years) for new
    unit, by drive-model. Right: TTF (top) and loss based on exponential
    devaluation, $\lambda=5$ (bottom), for best 11 drive-models.}
  \label{fig5}
\end{figure}
