...
\subsection{Overview}
The structure of the paper is as follows.  Section 1 summarizes the Backblaze data and the unique modeling challenges it presents.  Section 2 introduces the GFLP model as a mixture of two Weibull distributions.  In Section 3 we fit multiple specifications of the GFLP model to best model the Backblaze data.  We also discuss computational and modeling issues that suggests a final model with a common Weibull distribution for defective units and individual Weibull distributions for the wearout failure mode.  In Section 4 we compare hard drive brand models based on our final GFLP model.  Finally, in Section 5we review the GFLP model and propose future extensions.
...
The set of models to compare was chosen based on the data, interpretation of the model, as well as estimation considerations.  For all model specifications the location and scale parameter of the first failure mode was fixed across drive brand models.  Partly this was due to a lack of early failures in the data.  Moreover, though, from a consumer or business standpoint, comparing the lifetime distribution of infant mortality across hard drive brand models is not particularly informative.  The more practical questions are: is this hard drive likely to suffer from infant morality? and how long should it expect to run before it fails from wearout?  Assuming a constant failure distribution for infant mortality allows the first question to be answered since $\pi$'s are comparable across brands, which would not be the case if each drive brand model had a unique Weibull distribution fit to the early failure mode.  Flexibility in the parameters for the 2nd failure mode is necessary as initial plots of the data indicate heterogeneity across brands in the right tails of the failure distribution.  Going from a fixed model for all hard drive brands and gradually increasing the complexity of the model, we end up at Model 4, which has the most parameters.  Model 4 allows for the probability of infant mortality as well as the shape and scale parameters for the 2nd failure mode to vary by brand drive model.
