...
\begin{figure}[H]
    \centering
   \includegraphics[width=.8\textwidth]{fig/mod_compare_legend}
...
We consider the problem of selecting between drive-models. From a
...
\begin{align*}
  p(t_{d,new}|t) = \int p(\t_{d,new}|\theta_d) p(\theta_d|t) \, d\theta_d\\
\end{align*}
...
$s=1,...,S$, using the saved posterior draws for the model parameters.

Predicted time to failure does not account for depreciation. Assuming that
antiquated computer hardware tends to have less practical value, we
may want to use a different measure which will reward more the drive-models
that are very reliable in the first 0-5 years compared with those with a
larger number of early failures, but which may last 20 years.
