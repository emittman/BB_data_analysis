...
Failure distributions of high-reliability consumer products can be difficult
to assess; when field data are available, information may be limited. In this paper, we propose the use of Bayesian hierarchical modeling to borrow
information, improving inferences on lifetime
distributions where information is limited by right-censoring and
left-truncation.

The reliability of many engineered products follow a similar pattern. Relatively high rates of failure occur in early life ("infant mortality"). This can often be explained by defects from the manufacturing process. Following a period of "burn-in", after which the majority of the defective units have failed, failure rates flatten out during a period. Finally, after prolonged use, rates of failure increase due to wearout.  Ignoring this pattern in failure rates can lead to suboptimal decisions. While simple nonparametric methods are available, these are not suitable for forecasting.

\citet{chan} proposed the Generalized Limited Failure Population (GLFP) model. This model includes the Weibull as a special case. Additionally, it theorizes an unknown proportion of defective units which are susceptible to competing risks.\\ 

THIS NEEDS TO BE SEPARATED OUT
While the GLFP model provides flexibility, it can be difficult to fit with limited data. When multiple populations are of interest, comparisons based on GLFP model fits will be restricted to those with sufficient data. A naive approach would be to combine the data from all drive-models and fit a single GLFP model.  This model can be well-estimated, but this common GLFP model fits poorly.% the model captures drive-models with lots of observed failures, while the predicted time to failure for drive-models with less information is over or under estimated.  
