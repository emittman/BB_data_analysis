...
\sigma_1 \sim \op{Lognormal}\langle 7.4 \times 10 ^{-3} ,1.3 \times 10^2 \rangle, \quad
\sigma_2 \sim \op{Lognormal}\langle 7.4 \times 10 ^{-3} ,1.3 \times 10^2  \rangle $$
...
As we have discussed, while we prefer to allow all of the GLFP model parameters to be drive-model specific, there are practical issues with fitting the full model. For example, due to heavy left-truncation, there may be insufficient information for particular drive models to estimate all of the parameters. Furthermore, we prefer not to introduce unnecessary complexity into our analyses. Figure \ref{fig:fig2} provides a visual comparison for Models 1 through 4 by plotting the pointwise posterior median of time to failure. The large left panel shows the Kaplan-Meier nonparametric estimate of the same. the Kaplan-Meier estimates right side of Figure \ref{fig:fig2} shows that the set of estimated failure curves for all four models that we fit. The smaller plots correspond to Models 1 through 4 reading from left to right, top to bottom. Note that while all the paths for the posterior estimates necessarily pass through the origin, the nonparametric curves do not, but only because, for the Kaplan-Meier estimator, left-truncation disallows any kind of interpolation.

%Should we be brief discussing the plot?
These plots seem to suggest that Model 1 is far too naive, Model 2 is still too restrictive and Models 3 and 4 are fairly similar.
...
The question of which Model is best requires the choice of some criteria. From the point of view of an operator, the goal of an analysis like this is likely to rate manufacturers and/or to inform future purchaes based on predicted product performance. In this case, it seems that the best model would provide the most accurate prediction of lifetime for future units. One such metric is log posterior predictive density (lpd), $\log[p(t_new|t)]$. Thus we might choose a model as the one most likely to produce a high lpd, one maximizing $\op{E}_h \log[p(t_new|t)]$, where the expectation is with respect to the true distribution of $t$. Although $h$ is unknown, it can be estimated using leave-one-out cross-validation:
$$\frac{1}{G}\frac{1}{n_g}\sum_{g=1}^G \sum_{i=1}^{n_g} \log [p(t_{g,i}|t_{g,i}^L,c_{g,i},y_{-(g,i)})].$$ Using recently developed software provided in the \texttt{loo} package in \texttt{R},
