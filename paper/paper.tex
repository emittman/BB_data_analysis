...
Consumers have an interest in accurate assessment of product reliability. Toward this end, there is a need for models that reflect observable facts "on the ground", and methods which can make the most of available data and properly account for uncertainty.
...
We can compare the realizations of $y_{rep,g}$ to $y_g$ by using them to reestimate the nonparametric estimator. The resulting plots are displayed in figure \ref{post-pred-KM}. For most drive-models, the observed estimates appear to behave similarly to those derived from the replicated data. As discussed in \ref{subsec:ex1}, drive-model 14 has a discrepancy in the right tail, which we addressed there. Drive-moodel 4 also appears as an exception, which we discuss now.

To better understand the reason for the discrepancy between the observed data for drive-model 4 and its posterior predictive distribution, we plot the raw data using a version of the lifetime plot due to WHOM?? along with the truncation-adjusted nonparametric estimate of time to failure and the posterior estimate. The lifetime plot makes it clear that the large discrepancy between the posterior and the nonparametric estimate is driven by a small number of failures (4 or 5) that occur in the first quarter year. Indeed, if we ignore records with observation periods that include the first quarter, the modified nonparametric estimate falls within the posterior 90\% pointwise credible band. Our trouble with fitting this drive-model is that while there is evidence of infant mortality in the relatively few units that were observed in the first quarter, the problem seems to go away much quicker than the early failure mode (shared across drive models) would predict. 

Similar plots can help to illustrate the 
and what we call the ``global estimate," corresponding to $\op{GFLP}(\eta_pi, \mu_1,\sigma_1, \eta_{t_2} - \eta_{\sigma_2}\Phi^{-1}(.2), \eta_{\sigma_2})$. These are shown in figure \ref{}.
