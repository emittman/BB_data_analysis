...
#   # return(stan_data)
# }
run_mcmc_logodds_null <- function(dataset, chains = 4, iter = 10, warmup = iter/2, p = c(.5,.2)){
  require(rstan)
  rstan_options(auto_write = TRUE)
  options(mc.cores = parallel::detectCores())#format for Stan
  stan_data <- with(dataset,
  ...
  m <- stan_model(file = "../stan_models/glfp_lo_null_model.stan")
  s <- sampling(obj = m, data = stan_data, chains = chains, iter = iter, warmup = warmup, control = list(adapt_delta = .99), init=0)
  return(s)
  # return(stan_data)
}

run_mcmc_logodds_pis <- function(dataset, chains = 4, iter = 10, warmup = iter/2, p = c(.5,.2)){
  require(rstan)
  rstan_options(auto_write = TRUE)
  options(mc.cores = parallel::detectCores())#format for Stan
  stan_data <- with(dataset,
  ...
  m <- stan_model(file = "../stan_models/glfp_lo_pis_vary.stan")
  s <- sampling(obj = m, data = stan_data, chains = chains, iter = iter, warmup = warmup, control = list(adapt_delta = .99), init=0)
  return(s)
  # return(stan_data)
}

run_mcmc_logodds_tp2 <- function(dataset, chains = 4, iter = 10, warmup = iter/2, p = c(.5,.2)){
  require(rstan)
  rstan_options(auto_write = TRUE)
  options(mc.cores = parallel::detectCores())#format for Stan
  stan_data <- with(dataset,
  ...
  m <- stan_model(file = "../stan_models/glfp_lo_tp2_vary.stan")
  s <- sampling(obj = m, data = stan_data, chains = chains, iter = iter, warmup = warmup, control = list(adapt_delta = .99), init=0)
  return(s)
  # return(stan_data)
}
...
run_mcmc_extreme <- function(dataset, chains = 4, iter = 10, warmup = iter/2, p = c(.5,.2)){
  require(rstan)
  rstan_options(auto_write = TRUE)
  options(mc.cores = parallel::detectCores())#format for Stan
  stan_data <- with(dataset,
  ...
  m <- stan_model(file = "../stan_models/glfp_super_relaxed.stan")
  s <- sampling(obj = m, data = stan_data, chains = chains, iter = iter, warmup = warmup, control = list(adapt_delta = .999), init=0)
  return(s)
  # return(stan_data)
}
